"""Gemini free-tier provider (simulated).

Unified provider interface, with basic retry logic and tool-call suggestion.
"""
...
class GeminiFreeProvider(Provider):
    def generate(self, messages: List[Dict[str, Any]], tools: Optional[List[Dict]] = None, system: Optional[str] = None, progress_callback: Optional[callable] = None) -> Dict[str, Any]:
        self.logger.info("GeminiFreeProvider.generate called; messages=%d", len(messages))
        attempts = 0
        last_user = None
        ...
        while attempts < 3:
            ...
            try:
                time.sleep(0.02)
                reply = f"[GeminiFree simulated response] {content}"
                try:
                ...
                return {"text": reply, "tool_calls": tool_calls, "raw": {"provider": "gemini_free"}}
            except Exception as e:
                self.logger.exception("Gemini provider error on attempt %d: %s", attempts, e)
