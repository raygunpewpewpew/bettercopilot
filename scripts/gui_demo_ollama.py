#!/usr/bin/env python3
r"""Demo: open the native GUI and programmatically ask the Ollama panel a question.
...
message = "Ollama test: can you read me?"
print('Scheduling ollama ask:', message)

def do_ask():
    print('Sending ollama ask...')
    def on_result(res):
        try:
            print('\n--- OLLAMA DEMO RESULT ---')
            text = None
            ...
            print(text)
            try:
                if api.ai_panel_ollama:
                    api.ai_panel_ollama.append_message('assistant', text)
            except Exception:
                pass
        except Exception:
            traceback.print_exc()
    ...
    try:
        # Populate the Ollama panel UI so the question is visible to the user
        try:
            if api.ai_panel_ollama:
                try:
                    if hasattr(api.ai_panel_ollama, 'input'):
                        api.ai_panel_ollama.input.setText(message)
                    if hasattr(api.ai_panel_ollama, 'append_message'):
                        api.ai_panel_ollama.append_message('user', message)
                except Exception:
                    pass
        except Exception:
            pass

        api.run_ask(message, callback=on_result, error_callback=on_error, provider_override='ollama', response_panel=api.ai_panel_ollama)
    except Exception as e:
